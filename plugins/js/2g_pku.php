...
<!DOCTYPE html >
  <head>
	...
	<link type="text/css" rel="stylesheet" href="plugins/js/smoothness/jquery-ui.css" />
	<script type="text/javascript" src="plugins/js/external/jquery/jquery.js"></script>
	<script type="text/javascript" src="plugins/js/jquery-ui.js"></script>
	<script type="text/javascript" src="plugins/js/jquery-ui.min.js"></script>
	<link type="text/css" rel="stylesheet" href="plugins/js/jquery-ui.theme.css"/>
	<script>
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<td width=20%><b>End Date :</b>
					<input type="text" class="tanggal2">
     ...
     function configureDropDownLists(ddl1,ddl2) {
		 <?php
			...
			#query untuk object cluster/region/site
		 	$result = mysql_query("SELECT Distinct Cluster FROM Cluster2GDay WHERE Region='Sumbagteng' ORDER BY Cluster",$connect);
			while ($row = mysql_fetch_array($result)) {
				$clustername[] = "'".$row['Cluster']."'";
			}
			
			$result2 = mysql_query("SELECT Distinct Region FROM Region2GDay WHERE Region='Sumbagteng' ORDER BY Region",$connect);
			while ($row2 = mysql_fetch_array($result2)) {
				$Region[] = "'".$row2['Region']."'";
			}
			
			$result3 = mysql_query("SELECT Distinct SITEID FROM Site2GDay WHERE Region='Sumbagteng' ORDER BY SITEID",$connect);
			...
			$tgl = mysql_query("SELECT Distinct Date FROM Site2GDay WHERE Region='Sumbagteng' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
