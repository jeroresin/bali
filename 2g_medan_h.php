...
<!DOCTYPE html >
  ...
  <?
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 1 MONTH) from Region2GHour where Region='Sumbagut'",$connect);
		while ($row = mysql_fetch_array($qsd)) {
			$datemin = $row[0];
		}

		$qed = mysql_query("Select max(Date) from Region2GHour where Region='Sumbagut'",$connect);
  ...
  ?>
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<datalist id="ddl2">
						<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM ".$aggr."2GHour WHERE Region='Sumbagut' ORDER BY ".$aggrselect,$connect);
						while($row=mysql_fetch_array($res)){
     ...
     function configureDropDownLists(ddl1,ddl2) {
		 <?php
			...
			#query untuk object cluster/region/site
		 	$result = mysql_query("SELECT Distinct Cluster FROM Cluster2GHour WHERE Region='Sumbagut' ORDER BY Cluster",$connect);
			while ($row = mysql_fetch_array($result)) {
				$clustername[] = "'".$row['Cluster']."'";
			}
			
			$result2 = mysql_query("SELECT Distinct Region FROM Region2GHour WHERE Region='Sumbagut' ORDER BY Region",$connect);
			while ($row2 = mysql_fetch_array($result2)) {
				$Region[] = "'".$row2['Region']."'";
			}
			
			$result3 = mysql_query("SELECT Distinct SITEID FROM Site2GHour WHERE Region='Sumbagut' ORDER BY SITEID",$connect);
			...
			$tgl = mysql_query("SELECT Distinct Date FROM Cluster2GHour WHERE Region='Sumbagut' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_cluster[] = "'".$row['Date']."'";
			}
			
			$tgl = mysql_query("SELECT Distinct Date FROM Region2GHour WHERE Region='Sumbagut' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_region[] = "'".$row['Date']."'";
			}
			
			$tgl = mysql_query("SELECT Distinct Date FROM Site2GHour WHERE Region='Sumbagut' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_site[] = "'".$row['Date']."'";
			}
