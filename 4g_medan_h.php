...
<!--//==================== ibnurc@gmail.com @Huawei 2017===============//-->
  ...
  <?
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 14 DAY) from Region4GHour where Region='Sumbagut'",$connect);
		while ($row = mysql_fetch_array($qsd)) {
			$datemin = $row[0];
		}

		$qed = mysql_query("Select max(Date) from Region4GHour where Region='Sumbagut'",$connect);
	...
	$c1 = mysql_query("SELECT * FROM 4g_siteID_ceklist  WHERE status = 'swapped'", $connect);
	$num_rows_c1 = mysql_num_rows($c1);
	
	$c2 = mysql_query("SELECT * FROM 4g_siteID_ceklist WHERE status = 'not_swap'", $connect);
	$num_rows_c2 = mysql_num_rows($c2);
	
	if ($aggr == "Site") {
		$aggrselect = "SITEID";
		$colorch = "colors[8]";
		$limitbefore = mysql_query("SELECT Date, ".$aggrselect.", `RRC Setup Success Rate (Service)`, `E-RAB Setup Success Rate (All)`,  CSSR, SDR, IFHO, `Downlink Traffic Volume`, `Uplink Traffic Volume`, `LTE to WCDMA Redirection Success Rate`, `LTE to GERAN Redirection Success Rate`, `Cell Downlink Average Throughput`, `Cell Uplink Average Throughput`, `Avg Active User`, `Max Active User` FROM ".$aggr."4GHour WHERE ".$aggrselect."='".$objname."' AND Date >='".$sd."' AND Date <='".$ed."' And Region='SUMBAGUT' And Remarks='Ericsson' ORDER BY Date",$connect);
  ...
  ?>
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<datalist id="ddl2">
						<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM ".$aggr."4GHour WHERE Region='Sumbagut' ORDER BY ".$aggrselect,$connect);
						while($row=mysql_fetch_array($res)){
    ...
    <script>
      var customLabel = {
        ...
        var infoWindow = new google.maps.InfoWindow;
          ...
          downloadUrl('4g_markers.php', function(data) {
     ...
     function configureDropDownLists(ddl1,ddl2) {
		 <?php
			...
			#query untuk object cluster/region/site
		 	$result = mysql_query("SELECT Distinct Cluster FROM Cluster4GHour WHERE Region='Sumbagut' ORDER BY Cluster",$connect);
			while ($row = mysql_fetch_array($result)) {
				$clustername[] = "'".$row['Cluster']."'";
			}
			
			$result2 = mysql_query("SELECT Distinct Region FROM Region4GHour WHERE Region='Sumbagut' ORDER BY Region",$connect);
			while ($row2 = mysql_fetch_array($result2)) {
				$Region[] = "'".$row2['Region']."'";
			}
			
			//$result3 = mysql_query("SELECT Distinct ENODEBID FROM Site4GHour WHERE Region='Sumbagut' ORDER BY ENODEBID",$connect);
			//while ($row3 = mysql_fetch_array($result3)) {
				//$siteid[] = "'".$row3['ENODEBID']."'";
			//}

			$result3 = mysql_query("SELECT Distinct SITEID FROM Site4GHour WHERE Region='Sumbagut' ORDER BY SITEID",$connect);
			while ($row3 = mysql_fetch_array($result3)) {
				$siteid[] = "'".$row3[0]."'";
			}
			...
			$tgl = mysql_query("SELECT Distinct Date FROM Cluster4GHour WHERE Region='Sumbagut' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_cluster[] = "'".$row['Date']."'";
			}
			
			$tgl = mysql_query("SELECT Distinct Date FROM Region4GHour WHERE Region='Sumbagut' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_region[] = "'".$row['Date']."'";
			}
			
			$tgl = mysql_query("SELECT Distinct Date FROM Site4GHour WHERE Region='Sumbagut' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_site[] = "'".$row['Date']."'";
			}
