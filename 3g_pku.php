...
<!DOCTYPE html >
  ...
  <?
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qed = mysql_query("Select Max(Date) from Region3GDay where Region='Sumbagteng'",$connect);
		while ($row = mysql_fetch_array($qed)) {
			$datemax = date('Ymd',strtotime($row[0]));
		}

		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 1 MONTH) from Region3GDay where Region='Sumbagteng'",$connect);
  ...
  ?>
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<datalist id="ddl2">
						<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM ".$aggr."3GDay WHERE Region='Sumbagteng' ORDER BY ".$aggrselect,$connect);
						while($row=mysql_fetch_array($res)){
