...
<!DOCTYPE html >
  ...
  <?php
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 3 DAY) from 3g_clusterhourly",$connect);
		while ($row = mysql_fetch_array($qsd)) {
			$datemin = date('Ymd',strtotime($row[0]));
		}

		$qed = mysql_query("Select max(Date) from 3g_clusterhourly ",$connect);
	...
	if ($aggr == "Site") {
		...
		//$limitbefore = mysql_query("SELECT Date, ".$aggrselect.", `CSSR_CS`,`CSSR_PS`,`CSSR_HSDPA`,`CSSR_HSUPA`,`CCSR CS`,`CCSR PS`,`CCSR HSDPA`,`CCSR HSUPA`,`ISHO SR`,`IFHO Success Ratio`,`SHO SR`,`HSDPA_cell_average_throughput_Kbps`,`HSUPA_cell_average_throughput_Kbps`,`PS_cell_average_throughput_Kbps`,`Traffic_Voice_Erlang`,`Traffic_Video_Erlang`,`Payload PS (MByte)`,`Payload HSDPA (MByte)`,`Payload HSUPA (MByte)`,`CSSR_CS`,`CSSR_PS`,`CSSR_HSDPA`,`CSSR_HSUPA`,`CCSR CS`,`CCSR PS`,`CCSR HSDPA`,`CCSR HSUPA`,`ISHO SR`,`IFHO Success Ratio`,`SHO SR` FROM ".$aggr."3GDay WHERE ".$aggrselect."='".$objname."' And Date >=".$sd." AND Date <=".$ed." And Region = 'SUMBAGUT' ORDER BY Date",$connect);
			...
			while ($row = mysql_fetch_array($result)) {
				...
				$PayloadHSUPAMByte[]=$row['Payload HSUPA (MByte)'];
				*/
				//$tanggal[] = date("His",strtotime($row['Date']));
				$tanggal[] = date("Ymd",strtotime($row['Date'])).date("Hi",strtotime($row['Date']));
				//$tanggal[] = date("His",strtotime($row['hour']));
				$cluster = $row[1];
  ...
  ?>
	...
	<div id="main">
	  ...
	  <span style="font-size:30px;cursor:pointer" onclick="openNav()">&#9776;  3G Hour</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="./images/Telkomsel_Logo.svg.png" alt="tsel logo" height="42" width="100"><img src="./images/huawei.gif" alt="huawei logo" height="42" width="45">
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<datalist id="ddl2">
						<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM 3G_".$aggr."hourly ORDER BY ".$aggrselect,$connect);
						while($row=mysql_fetch_array($res)){
