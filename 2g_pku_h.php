...
<!DOCTYPE html >
  ...
  <?
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qed = mysql_query("Select max(Date) from Region2GHour where Region='Sumbagteng'",$connect);
		while ($row = mysql_fetch_array($qed)) {
			$datemax = $row[0];
		}

		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 14 Day) from Region2GHour where Region='Sumbagteng'",$connect);
	...
	if ($aggr == "Site") {
		$aggrselect = "SITEID";
		$colorch = "colors[8]";
		$limitbefore = mysql_query("SELECT Date, ".$aggrselect.", `TCH_BLOCK_(%)`, `SD_BLOCK_(%)`, SDSR, `TBF_DL_Est_(%)`, `TCH_DROP_CALL_(%)`, `TBF_COMP_(%)`, HOSR, `DL_QUAL_050_(%)`, `Traffic (TCH) erl`, `SDCCH_TRAFFIC`,  `PAYLOAD_GPRS_Mbyte`, `PAYLOAD_EDGE_Mbyte` FROM ".$aggr."2GHour WHERE ".$aggrselect."='".$objname."' And Remarks='Ericsson' And Date >='".$sd."' AND Date <='".$ed."' And Region = 'SUMBAGTENG' ORDER BY Date",$connect);
	
		$result = mysql_query("SELECT Date, ".$aggrselect.", `TCH_BLOCK_(%)`, `SD_BLOCK_(%)`, SDSR, `TBF_DL_Est_(%)`, `TCH_DROP_CALL_(%)`, `TBF_COMP_(%)`, HOSR, `DL_QUAL_050_(%)`, `Traffic (TCH) erl`, `SDCCH_TRAFFIC`,  `PAYLOAD_GPRS_Mbyte`, `PAYLOAD_EDGE_Mbyte` FROM ".$aggr."2GHour WHERE ".$aggrselect."='".$objname."' AND Date >='".$sd."' AND Date <='".$ed."' And Region = 'SUMBAGTENG' And Remarks='Huawei' ORDER BY Date",$connect);
		...
		$limitbefore = mysql_query("SELECT Date, ".$aggrselect.", `TCH_BLOCK_(%)`, `SD_BLOCK_(%)`, SDSR, `TBF_DL_Est_(%)`, `TCH_DROP_CALL_(%)`, `TBF_COMP_(%)`, HOSR, `DL_QUAL_050_(%)`, `Traffic (TCH) erl`, `SDCCH_TRAFFIC`,  `PAYLOAD_GPRS_Mbyte`, `PAYLOAD_EDGE_Mbyte` FROM ".$aggr."2GHour WHERE ".$aggrselect."='".$objname."' And Date >='".$sd."' AND Date <='".$ed."' And Region = 'SUMBAGTENG' ORDER BY Date",$connect);
				...
				$qbaseline = mysql_query("SELECT Region, Cluster, Week, `TCH Block (%)`, `SD Block (%)`, `SDSR (%)`, `TBF DL est (%)`, `TCH Drop Call (%)`, `TBF Comp (%)`, `HOSR (%)`, `DL Qual 0-5 (%)`, `TCH_TRAFFIC`, `SDCCH_TRAFFIC`, `GPRS Payload`, `EGDE Payload` from 2gbaseline where Region='Sumbagteng' and Cluster='".$objname."'",$connect);
  ...
  ?>
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<datalist id="ddl2">
					<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM ".$aggr."2GHour WHERE Region='Sumbagteng' ORDER BY ".$aggrselect,$connect);
						while($row=mysql_fetch_array($res)){
    ...
    <script>
     ...
     function configureDropDownLists(ddl1,ddl2) {
		 <?php
			...
			$result3 = mysql_query("SELECT Distinct SITEID FROM Site2GHour WHERE Region='Sumbagteng' ORDER BY SITEID",$connect);
			...
			$tgl = mysql_query("SELECT Distinct Date FROM Cluster2GHour WHERE Region='Sumbagteng' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_cluster[] = "'".$row['Date']."'";
			}
			
			$tgl = mysql_query("SELECT Distinct Date FROM Region2GHour WHERE Region='Sumbagteng' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_region[] = "'".$row['Date']."'";
			}
			
			$tgl = mysql_query("SELECT Distinct Date FROM Site2GHour WHERE Region='Sumbagteng' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_site[] = "'".$row['Date']."'";
			}
