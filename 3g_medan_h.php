...
<!--//==================== ibnurc@gmail.com @Huawei 2017===============//-->
  ...
  <?
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 14 DAY) from Region3GHour where Region='Sumbagut'",$connect);
		while ($row = mysql_fetch_array($qsd)) {
			$datemin = $row[0];
		}

		$qed = mysql_query("Select max(Date) from Region3GHour where Region='Sumbagut'",$connect);
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<input type="text" name="optobjname" list="ddl2" autocomplete="off"/>
				<datalist id="ddl2">
						<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM ".$aggr."3GHour WHERE Region='Sumbagut' ORDER BY ".$aggrselect,$connect);
						while($row=mysql_fetch_array($res)){
     ...
     function configureDropDownLists(ddl1,ddl2) {
		 <?php
			...
			#query untuk object cluster/region/site
		 	$result = mysql_query("SELECT Distinct Cluster FROM Cluster3GHour WHERE Region='SUMBAGUT' ORDER BY Cluster",$connect);
			while ($row = mysql_fetch_array($result)) {
				$clustername[] = "'".$row['Cluster']."'";
			}
			
			$result2 = mysql_query("SELECT Distinct Region FROM Region3GHour WHERE Region='SUMBAGUT' ORDER BY Region",$connect);
			while ($row2 = mysql_fetch_array($result2)) {
				$Region[] = "'".$row2['Region']."'";
			}
			
			$result3 = mysql_query("SELECT Distinct SITEID FROM Site3GHour WHERE Region='SUMBAGUT' ORDER BY SITEID",$connect);
			...
			$tgl = mysql_query("SELECT Distinct Date FROM Cluster3GHour WHERE Region='SUMBAGUT' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_cluster[] = "'".$row['Date']."'";
			}
			
			$tgl = mysql_query("SELECT Distinct Date FROM Region3GHour WHERE Region='SUMBAGUT' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_region[] = "'".$row['Date']."'";
			}
			
			$tgl = mysql_query("SELECT Distinct Date FROM Site3GHour WHERE Region='SUMBAGUT' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
				$tgl_site[] = "'".$row['Date']."'";
			}
