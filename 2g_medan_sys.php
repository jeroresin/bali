...
<!--//==================== ibnurc@gmail.com @Huawei 2017===============//-->
  ...
  <?
	...
	if ($aggr == "Site") {
		...
		WHERE SiteID='".$objname."' AND Region = 'SUMBAGUT' AND Date >='".$sd."' AND Date <='".$ed."' 
		GROUP BY Date
		...
		?>
			...
			chart = new Highcharts.Chart({
				  chart: {
					 ...
					 events: {
						load:function(){
										this.renderer.image('./images/tselhuawei.png', '76%', 5, 100, 30).add();
									} 
					}
				  },
				  credits: {
		...
		<tr>
			...
			<datalist id="ddl2">
						...
						if ($aggrselect =="SITEID") {
							...
							$res = mysql_query("SELECT Distinct ".$aggrselect." FROM ".$aggr."2GDay_System WHERE Region='Sumbagut' ORDER BY ".$aggrselect,$connect);
							while($row=mysql_fetch_array($res)){
    ...
    <script>
     ...
     function configureDropDownLists(ddl1,ddl2) {
		 <?php
			...
			$result3 = mysql_query("SELECT Distinct SiteID FROM Site2GDay_System WHERE Region='Sumbagut' ORDER BY SiteID",$connect);
			...
			$tgl = mysql_query("SELECT Distinct Date FROM Site2GDay_System WHERE Region='Sumbagut' ORDER BY Date",$connect);
			while ($row = mysql_fetch_array($tgl)) {
