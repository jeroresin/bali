...
<!DOCTYPE html >
  ...
  <?php
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 1 MONTH) from 2g_clusterdaily",$connect);
		while ($row = mysql_fetch_array($qsd)) {
			$datemin = date('Ymd',strtotime($row[0]));
		}

		$qed = mysql_query("Select max(Date) from 2g_clusterdaily ",$connect);
	...
	*/
	...
	if ($aggr == "Site") {
		...
		//$limitbefore = mysql_query("SELECT Date, SITEID, `cluster`,`CSSR_CS(%)`,`CSSR_PS(%)`,`CSSR_HSDPA(%)`,`CSSR_HSUPA(%)`,`CCSR_CS(%)`,`CCSR_PS(%)`,`CCSR_HSDPA(%)`, `CCSR_HSUPA(%)`,`SHO_%`,`ISHO_%`,`IFHO_%` ,`HSDPA_Cell_Avg_Throughput_Kbps`,`HSUPA_Cell_Avg_Throughput_Kbps`,`Traffic_Voice`,`PS_Payload_Mbyte`,`HSDPA_Payload_Mbyte`,`HSUPA_Payload_Mbyte` ,`HSDPA_Cell_Avg_Throughput_Kbps` ,`HSUPA_Cell_Avg_Throughput_Kbps` FROM 2g_Sitedaily WHERE ".$aggrselect."='".$objname."' And region= 'Bali' And Date >='".$sd."' AND Date <='".$ed."' ORDER BY Date",$connect);
	$limitbefore = mysql_query("SELECT Date, SiteID,  `TCH_Block`, `SDCCH_Block`, `SDSR_%`, `DL_TBF_Est_SR_%`, `TDR_Nom`, `TBF_Completion_SR_%`, `HOSR_%`, `%_DL_RX_Qual_0_5`, `TCH_Traffic_Erl`, `SDCCH_Traffic_Erl`,  `Payload_GPRS_MByte`, `Payload_EGPRS_MByte` from 2g_sitedaily  WHERE SiteID='".$objname."' AND Date >=".$sd." AND Date <=".$ed."   ORDER BY Date",$connect);;
	
		$result = mysql_query("SELECT Date, SiteID,  `TCH_Block`, `SDCCH_Block`, `SDSR_%`, `DL_TBF_Est_SR_%`, `TDR_Nom`, `TBF_Completion_SR_%`, `HOSR_%`, `%_DL_RX_Qual_0_5`, `TCH_Traffic_Erl`, `SDCCH_Traffic_Erl`,  `Payload_GPRS_MByte`, `Payload_EGPRS_MByte` FROM 2g_Sitedaily where SiteID='".$objname."' ",$connect);
		...
		$result = mysql_query("SELECT Date, ".$aggrselect.", `CSSR_CS`,`CSSR_PS`,`CSSR_HSDPA`,`CSSR_HSUPA`,`CCSR CS`,`CCSR PS`,`CCSR HSDPA`,`CCSR HSUPA`,`ISHO SR`,`IFHO Success Ratio`,`SHO SR`,`HSDPA_cell_average_throughput_Kbps`,`HSUPA_cell_average_throughput_Kbps`,`PS_cell_average_throughput_Kbps`,`Traffic_Voice_Erlang`,`Traffic_Video_Erlang`,`Payload PS (MByte)`,`Payload HSDPA (MByte)`,`Payload HSUPA (MByte)`,`CSSR_CS`,`CSSR_PS`,`CSSR_HSDPA`,`CSSR_HSUPA`,`CCSR CS`,`CCSR PS`,`CCSR HSDPA`,`CCSR HSUPA`,`ISHO SR`,`IFHO Success Ratio`,`SHO SR` FROM 3g_regiondaily WHERE ".$aggrselect."='".$objname."' AND Date >=".$sd." AND Date <=".$ed." And Region = 'SUMBAGUT' And Vendor='Huawei' ORDER BY Date",$connect);
			...
			while ($row = mysql_fetch_array($result)) {
				...
				*/
				...
				$aggrselect = $aggr;
		$colorch = "colors[0]";
		//$limitbefore = mysql_query("SELECT Date, ".$aggrselect.", `TCH_BLOCK_(%)`, `SD_BLOCK_(%)`, SDSR, `TBF_DL_Est_(%)`, `TCH_DROP_CALL_(%)`, `TBF_COMP_(%)`, HOSR, `DL_QUAL_050_(%)`, `Traffic (TCH) erl`, `SDCCH_TRAFFIC`,  `PAYLOAD_GPRS_Mbyte`, `PAYLOAD_EDGE_Mbyte` FROM 2g_clusterdaily WHERE ".$aggrselect."='".$objname."' And Date >=".$sd." AND Date <=".$ed."  ORDER BY Date",$connect);
	 $limitbefore = mysql_query("select Date, Cluster, `SDSR(%)`,`HOSR(%)`,`DL_TBF_Est_SR(%)`,`TBF_Completion_SR(%)`,`TDR(%)`,`TCH_Block`,`SDCCH_Blocking_Rate(%)`,`TCH_Traffic(Erl)`,SDCCH_Traffic,`DL_QUAL_(6-7)`,`Payload_GPRS_MByte`,`Payload_EGPRS_MByte` from 2g_clusterdaily WHERE ".$aggrselect."='".$objname."' AND Date >=".$sd." AND Date <=".$ed."   ORDER BY Date",$connect);;
		...
		//$result = mysql_query("SELECT Date, ".$aggrselect.", `TCH_BLOCK_(%)`, `SD_BLOCK_(%)`, SDSR, `TBF_DL_Est_(%)`, `TCH_DROP_CALL_(%)`, `TBF_COMP_(%)`, HOSR, `DL_QUAL_050_(%)`, `Traffic (TCH) erl`, `SDCCH_TRAFFIC`,  `PAYLOAD_GPRS_Mbyte`, `PAYLOAD_EDGE_Mbyte` FROM 2g_clusterdaily WHERE ".$aggrselect."='".$objname."' AND Date >=".$sd." AND Date <=".$ed." And Vendor='Huawei'  ORDER BY Date",$connect);
		$result = mysql_query("select Date, Cluster, `SDSR(%)`,`HOSR(%)`,`DL_TBF_Est_SR(%)`,`TBF_Completion_SR(%)`,`TDR(%)`,`TCH_Block`,`SDCCH_Blocking_Rate(%)`,`TCH_Traffic(Erl)`,SDCCH_Traffic,`DL_QUAL_(6-7)`,`Payload_GPRS_MByte`,`Payload_EGPRS_MByte`,`TCH_Drop`,`DL_QUAL_(6-7)` from 2g_clusterdaily WHERE ".$aggrselect."='".$objname."'  ORDER BY Date",$connect);;
		
		while ($row = mysql_fetch_array($result)) {
			$tchblockrate[] = $row['TCH_Block'];
			$payloadgprs[] = $row['Payload_GPRS_MByte'];
			$payloadedge[] = $row['Payload_EGPRS_MByte'];
			$traffictch[] = $row['TCH_Traffic(Erl)'];
			$trafficsd[] = $row['SDCCH_Traffic'];
			$sdsr[] = $row['SDSR(%)'];
			$hosr[] = $row['HOSR(%)'];
			$tbfcomp[] = $row['TBF_Completion_SR(%)'];
			$tbfdlest[] = $row['DL_TBF_Est_SR(%)'];
			$sdblockrate[] = $row['SDCCH_Blocking_Rate(%)'];
  ...
  ?>
	...
	<div id="main">
	  ...
	  <span style="font-size:30px;cursor:pointer" onclick="openNav()">&#9776; 2G KPI</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="./images/Telkomsel_Logo.svg.png" alt="tsel logo" height="42" width="100"><img src="./images/huawei.gif" alt="huawei logo" height="42" width="45">
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<datalist id="ddl2">
						<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM 2G_".$aggr."daily ORDER BY ".$aggrselect,$connect);
						while($row=mysql_fetch_array($res)){
