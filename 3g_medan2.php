...
<!DOCTYPE html >
  ...
  <?php
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 1 MONTH) from 3g_nodebdaily",$connect);
		while ($row = mysql_fetch_array($qsd)) {
			$datemin = date('Ymd',strtotime($row[0]));
		}

		$qed = mysql_query("Select max(Date) from 3g_nodebdaily ",$connect);
	...
	*/
	...
	if ($aggr == "Site") {
		$aggrselect = "SITEID";
		$colorch = "colors[8]";
		//$limitbefore = mysql_query("SELECT Date, SITEID, `CSSR_CS(%)`,`CSSR_PS(%)`,`CSSR_HSDPA(%)` FROM 3g_nodebdaily WHERE  Date >= '2018-02-20' and Date <= '2018-02-28' order by Date",$connect);
		$limitbefore = mysql_query("SELECT Date, SITEID, `CSSR_CS(%)`,`CSSR_PS(%)`,`CSSR_HSDPA(%)`,`CSSR_HSUPA(%)`,`CCSR_CS(%)`,`CCSR_PS(%)`,`CCSR_HSDPA(%)`, `CCSR_HSUPA(%)`,`SHO_%`,`ISHO_%`,`IFHO_%` ,`HSDPA_Cell_Avg_Throughput_Kbps`,`HSUPA_Cell_Avg_Throughput_Kbps` FROM 3g_nodebdaily WHERE  Date >='".$sd."' AND Date <='".$ed."' ORDER BY Date",$connect);
	
		$result = mysql_query("SELECT Date, SITEID, `CSSR_CS(%)`,`CSSR_PS(%)`,`CSSR_HSDPA(%)`,`CSSR_HSUPA(%)`,`CCSR_CS(%)`,`CCSR_PS(%)`,`CCSR_HSDPA(%)`,`CCSR_HSUPA(%)`,`SHO_%`,`ISHO_%`,`IFHO_%`,`HSDPA_Cell_Avg_Throughput_Kbps`,`HSUPA_Cell_Avg_Throughput_Kbps`,`Traffic_Voice`,`PS_Payload_Mbyte`,`HSDPA_Payload_Mbyte`,`HSUPA_Payload_Mbyte` FROM 3g_nodebdaily where SITEID ='".$objname."'",$connect);
		$bln = array();
		...
		$limitbefore = mysql_query("SELECT Date, ".$aggrselect.", `CSSR_CS`,`CSSR_PS`,`CSSR_HSDPA`,`CSSR_HSUPA`,`CCSR CS`,`CCSR PS`,`CCSR HSDPA`,`CCSR HSUPA`,`ISHO SR`,`IFHO Success Ratio`,`SHO SR`,`HSDPA_cell_average_throughput_Kbps`,`HSUPA_cell_average_throughput_Kbps`,`PS_cell_average_throughput_Kbps`,`Traffic_Voice_Erlang`,`Traffic_Video_Erlang`,`Payload PS (MByte)`,`Payload HSDPA (MByte)`,`Payload HSUPA (MByte)`,`CSSR_CS`,`CSSR_PS`,`CSSR_HSDPA`,`CSSR_HSUPA`,`CCSR CS`,`CCSR PS`,`CCSR HSDPA`,`CCSR HSUPA`,`ISHO SR`,`IFHO Success Ratio`,`SHO SR` FROM ".$aggr."3GDay WHERE ".$aggrselect."='".$objname."' And Date >=".$sd." AND Date <=".$ed." And Region = 'SUMBAGUT' ORDER BY Date",$connect);
			...
			while ($row = mysql_fetch_array($qbaseline)) {
				...
				$b_IFHOSR=$row['IFHO'];
				$b_SHOSR=$row['SHO'];
				$b_HSDPAcellthp=$row['HSDPA Throughput'];
				$b_HSUPAcellthp=$row['HSUPA Throughput'];
				...
				$b_PayloadPSMByte=$row['Payload_PS_R99_DL_Mbyte'] + $Row['Payload_PS_R99_UL_Mbyte'];
				$b_PayloadHSDPAMByte=$row['payload_HSDPA_Mbyte'];
				$b_PayloadHSUPAMByte=$row['payload_HSUPA_Mbyte'];
  ...
  ?>
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			<td width=20%><form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
				<b>Level :</b>
				<select  name="optaggr" id="ddl1" onchange="configureDropDownListsdong(this,document.getElementById('ddl2','ddl3','ddl4'))" >
						<option value="<?=$aggr?>" selected><?=$aggr?></option>
						<option value="<?=$aggr2?>"> Region </option>
						<option value="<?=$aggr3?>">Cluster </option>
			...
			<input type="text" name="optobjname" list="ddl2" autocomplete="on"/>
			<datalist id="ddl2">
			
						
						<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM 3g_nodebdaily order by SITEID",$connect);
    ...
    <script>
     var customLabel = {
        ...
        not_swap: {
		  icon: 'http://www.googlemapsmarkers.com/v1/N/0099FF/',
		  warna: '#e60000'
        }
      };
	  ...
	  function roundNumber(rnum, rlength) { 
		...
		<?php 
		$result3 = mysql_query("SELECT Distinct SITEID FROM 3g_nodebdaily ORDER BY SITEID",$connect);
			while ($row3 = mysql_fetch_array($result3)) {
				$siteid[] = "'".$row3['SITEID']."'";
				
		$tgl = mysql_query("SELECT Distinct Date FROM 3g_nodebdaily ORDER BY Date",$connect);
