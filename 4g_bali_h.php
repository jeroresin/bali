...
<!DOCTYPE html >
  ...
  <?php
	...
	//	$aggr = $sd = $ed = $objname = $aggrselect = "";
		...
		$qsd = mysql_query("Select DATE_SUB(Max(Date), INTERVAL 3 DAY) from 4g_clusterhourly",$connect);
		while ($row = mysql_fetch_array($qsd)) {
			$datemin = date('Ymd',strtotime($row[0]));
		}

		$qed = mysql_query("Select max(Date) from 4g_clusterhourly ",$connect);
	...
	*/
	...
	if ($aggr == "Site") {
		...
		while ($row = mysql_fetch_array($result)) {
			...
			//$tanggal[] = date("Hi",strtotime($row['hour']));
  ...
  ?>
	...
	<div id="main">
	  ...
	  <span style="font-size:30px;cursor:pointer" onclick="openNav()">&#9776; 4G Hour</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="./images/Telkomsel_Logo.svg.png" alt="tsel logo" height="42" width="100"><img src="./images/huawei.gif" alt="huawei logo" height="42" width="45">
	...
	<table width=100% Align="center" bgcolor="#A4A4A4">
		<tr>
			...
			<datalist id="ddl2">
						<?php
						$res = mysql_query("SELECT Distinct ".$aggrselect." FROM 4G_".$aggr."hourly ORDER BY ".$aggrselect,$connect);
						while($row=mysql_fetch_array($res)){
